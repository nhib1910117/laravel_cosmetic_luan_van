...
@section('content')
    ...
    <div class="col-md-12">
        <div class="x_panel">
            <div class="x_title">
                <h2>{{isset($product_edit) ? "Update Product": ""}}</h2>
            ...
            <div class="x_content">
                ...
                <form action="{{isset($product_edit) ? route('product.update',$product_edit->id) : route('product.store') }}"  method="POST" class="form-label-left input_mask" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6 col-sm-6 form-group">
                        <select class="select2_single form-control" name="bra_id" >
                                @foreach($brands as $brand)
                                    <option <?php if(isset($product_edit) && $product_edit->bra_id==$brand->id) { ?>
                                        selected
                                <?php   }elseif(old('bra_id')==$brand->id){ ?>
                                        selected
                                <?php  } ?> value="{{$brand->id}}">{{$brand->name}}</option>
                                @endforeach
                            </select> 
                    </div>
                    <div class="col-md-12 col-sm-12  form-group">
